...
This dissertation addresses that gap with a forward-looking simulation framework. Given only a root tweet and a population of historically grounded agents, the system generates a synthetic reply thread and evaluates its realism against observed threads using distributional fidelity metrics~\citep{composta2025simulating,butler2024misinformation}. The focus is not to recover ``true'' internal psychological states, but to reproduce observable reaction patterns in language and thread structure.
...
As a result, there is limited work that can simultaneously (i) run in a strictly forward mode from a root tweet, (ii) generate realistic natural-language replies, and (iii) support quantitative validation against real thread-level distributions across sentiment, politics, emotion, aggression, timing, and structure.
...
\item \textbf{A scalable validation framework}: 100-thread evaluation with per-thread metrics and figures covering sentiment, political stance, emotion, aggression, timing, and structural properties of generated threads.
...
\subsection{Few-Shot Thread Grounding}

The final component of the prompt engineering fix was few-shot thread grounding: up to eight real tweet texts from the target thread were sampled and injected into each agent's prompt as tone-anchoring examples. This grounding provided the model with concrete examples of how participants actually communicate in the thread, naturally encoding the thread's sentiment range, vocabulary register, and hostility level. Empirically, few-shot grounding was the most effective single change for reducing the sentiment bias identified in Section~4.5.1, because the real examples prevented the model from defaulting to uniform negativity~\citep{park2023generative}. The ablation study (Section~4.6) separately tested the system in 0-shot mode (with few-shot grounding disabled) to measure the contribution of this component; the results confirmed that few-shot grounding provides a meaningful improvement in sentiment calibration, though the system remains functional without it.

\section{Phase 6: Ablation Study}

To identify which design choices most contributed to simulation quality, a systematic ablation study was conducted in 0-shot mode (with few-shot grounding disabled) across 21 parameter configurations, each run across 10 diverse threads from the USC dataset (210 total simulations). The 0-shot constraint was imposed deliberately to test which parameters mattered most when the model had no access to real thread replies. Configurations were constructed as single-parameter ablations from the Phase~5 baseline --- changing exactly one parameter at a time --- plus five multi-parameter combinations designed to test hypotheses about parameter interactions.
...
The final 100-thread simulation runs reported in the Results section used the full Phase~5 parameter set, including both few-shot grounding and vocabulary injection, as these produced the best overall results in the pipeline validation.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{dissertation_figures/evaluation_100threads/07_depth_comparison.png}
\caption{Thread depth comparison: real threads are flat (depth~1) because the dataset does not preserve reply chains; simulated threads exhibit nested structure with mean max depth of 5.4.}
